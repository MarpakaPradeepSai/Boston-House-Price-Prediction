...
if st.button('Predict 🔍'):
    if input_data is not None:
        ...
        # Display the result in a styled box with smaller size
        st.markdown(f"""
            <div style="background-color: green; padding: 10px; text-align: center; border-radius: 10px; width: 250px; margin: auto;">
                <h4 style="color: white;"><strong>Predicted Median Value</strong></h4>
                <p style="font-size: 18px; color: white;">${median_value:.2f}</p>
            </div>
        """, unsafe_allow_html=True)
    else:
